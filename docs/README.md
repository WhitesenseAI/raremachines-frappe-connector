# RareMachines for Frappe CRM — installation and use
...
[RareMachines](https://whitesense.in), so your team can work the CRM from
WhatsApp — ask about leads and deals, and make changes, in chat. Your Frappe
site stays the system of record. RareMachines reads from it when you ask, and
...
- A RareMachines account. Sign up at [whitesense.in](https://whitesense.in).
- The site must be reachable from the internet over **https**. RareMachines
...
There is nothing to configure. The app talks to RareMachines at
...
**From RareMachines** — go to Connections, choose *Connect Frappe*, enter your
...
use the RareMachines shortcut on your CRM workspace) and click *Connect with
RareMachines*.

When it works, RareMachines Settings shows **Active**. Behind the scenes the app
creates an OAuth Client named *RareMachines* on your site — that is what lets
...
1. In RareMachines, open **Connections** and click *Connect my Frappe account*.
...
Your business records are never copied to RareMachines. Full detail:
...
| Message | What to do |
|---|---|
| That RareMachines workspace is already linked to a different Frappe site | The workspace points at another site. In RareMachines, disconnect that site first — or tick **Replace** when you link, which drops the old link and every teammate's personal login for it. |
| This Frappe site is already linked to a different RareMachines workspace | Someone else linked this site. Ask that workspace's admin to disconnect, or join their workspace instead of linking the site again. |
| Connect link expired | The link is single-use and short-lived. Click *Connect with RareMachines* again. |
| Could not reach RareMachines | Your Frappe site could not make an outbound connection. Check egress/firewall rules allow it to reach `whitesense.in`. |
| RareMachines could not reach this site at the URL it was given | RareMachines calls your site back during linking to prove it is reachable at the URL you entered. Check the site is reachable from the internet at that exact URL, including https. |
| RareMachines rejected this site's URL | The URL must be a public https address. Private, loopback and internal addresses are refused on purpose. |
...
| RareMachines is not configured correctly on this site | Contact support — this one is on our side, not yours. |
...
- **remove the RareMachines shortcut** it added to your CRM workspace.

Nothing is left for you to clean by hand. Disconnecting inside RareMachines as
