...
"""
List the CRM users on this site, so a RareMachines workspace admin can see who
already works in this Frappe site and invite them.

This only ever SUGGESTS invitations. It never provisions a RareMachines account and
never grants anyone access — a human admin still clicks Invite on the RareMachines
...
User who can read CRM Lead. That is real personal data crossing to RareMachines, and
...
Called by RareMachines's SERVER, never by a browser and never from a logged-in Desk
session (the RareMachines admin viewing the picker need not be logged into Frappe at
...
# RareMachines signs `timestamp.body`; anything older than this is refused as a
...
def _verify_install_signature() -> None:
	...
	if not secret:
		frappe.throw(_("This site is not paired with RareMachines."), frappe.PermissionError)
...
# allow_guest because RareMachines calls this server-to-server, with no Frappe
...
def list_crm_users() -> dict[str, Any]:
	...
	nothing here is a security boundary — RareMachines's own invite flow still
...
# allow_guest for the same reason as `list_crm_users`: RareMachines calls this
# server-to-server with no Frappe session, authenticated by the per-site HMAC.
@frappe.whitelist(allow_guest=True, methods=["POST"])  # nosemgrep: guest-whitelisted-method
def verify_install() -> dict[str, Any]:
	"""
	Prove to RareMachines that this site is genuinely reachable at the URL claimed
	for it, by echoing an HMAC over a nonce RareMachines chose.

	This exists because RareMachines's `pair/complete` previously accepted a
